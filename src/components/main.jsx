...
export default function Main (){
    ...
    const getEmployeeData = async () => {
        try {
            setLoading(true);
            let response =await fetch(`http://localhost:3001/employeeDetails?_page=${page}&_limit=5`);
    ...
    return (
        ...
        {
            loading ? (<h1>Loading</h1>) : error ? (<h1>Something went wrong</h1>) : (<MakeTable allEmployeeData={allEmployeeData} handlePage={handlePage} page={page} lastPage={lastPage}/>)
        }
        </>
    )
}